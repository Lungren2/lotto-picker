...
export function HistoryAnalysis() {
  ...
  const frequencyData = useMemo(() => {
    ...
    const totalNumbers = entries.reduce((sum, entry) => sum + entry.numbers.length, 0)
  ...
  const { mostFrequent, leastFrequent } = useMemo(() => {
    ...
    const leastFrequent = nonZero.length > 5 
      ? nonZero.slice(-5).reverse() 
      : []
  ...
  const patterns = useMemo(() => {
    ...
    const pairCounts: Record<string, { pair: [number, number]; count: number }> = {}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              >
                ...
                {mostFrequent.length > 0 && (
                  <motion.div variants={itemVariants} className="space-y-2">
                    <h3 className="text-sm font-medium">Most Frequent Numbers</h3>
                ...
                {leastFrequent.length > 0 && (
                  <motion.div variants={itemVariants} className="space-y-2">
                    <h3 className="text-sm font-medium">Least Frequent Numbers</h3>
                    <div className="flex flex-wrap gap-2">
          ...
          <TabsContent value="heatmap" className="space-y-4">
            <AnimatePresence mode="wait">
              ...
              >
                <motion.div variants={itemVariants} className="space-y-2">
                  <h3 className="text-sm font-medium">Number Frequency Heatmap</h3>
                  <div className="grid grid-cols-10 gap-1">
                    {frequencyData.map(({ number, count, percentage }) => {
                      // Calculate color intensity based on frequency
                      const intensity = Math.min(100, Math.max(0, percentage * 5))
