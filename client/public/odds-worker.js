// odds-worker.js
// Web Worker for background odds calculations
 ...
 * @returns {number} Factorial of n (as number if possible, otherwise converted from BigInt)
 ...
 *
 * @param {number} n Number to calculate factorial for
 * @param {number} startValue Starting value for calculation (from cache)
 * @returns {number} Factorial of n
 */
function calculateFactorialWithChunks(n, startValue) {
 ...
 *
 * @param {number} n Number to calculate factorial for
 * @returns {bigint} Factorial of n as BigInt
 */
 ...
 * @param {number} n Number to calculate log(n!) for
 ...
 * @returns {number} Natural logarithm of combinations(n,r)
 */
...
const hypergeometricCache = {}

/**
 * Calculate hypergeometric probability with memoization and error handling
 * @param {number} k Number of successes in the sample
 ...
 * @param {number} probability Single set probability
 * @param {number} numSets Number of sets
 * @returns {number} Adjusted probability
 */
function adjustedProbability(probability, numSets) {
...
/**
 * Calculate odds for all possible match counts
 * @param {number} quantity Number of items in a set
 * @param {number} maxValue Maximum value for the items
 * @param {number} numSets Number of sets to generate
 * @returns {Object} Odds data
 */
function calculateOdds(quantity, maxValue, numSets) {
  ...
  for (let k = 0; k <= quantity; k++) {
    // Calculate probability using hypergeometric function
...
self.addEventListener("message", function (e) {
  ...
  if (data.command === "calculate") {
    try {
      // Extract parameters
      const { quantity, maxValue, numSets } = data

      // Calculate odds
      const result = calculateOdds(quantity, maxValue, numSets)
