/**
 * TypeScript implementation of the Mersenne Twister (MT19937) algorithm
...
export class MersenneTwister {
   ...
   * @param seed The seed value (defaults to current timestamp if not provided)
   */
   ...
   */
  public intRange(min: number = 1, max: number = 100): number {
    if (min > max) {
   ...
   */
  public randomInclusive(): number {
    return this.int32() * (1.0 / 4294967295.0); // Divide by 2^32 - 1
  }
  
  /**
   * Generate a random float in the range (0, 1) with 32-bit resolution
   * @returns Random float between 0 (exclusive) and 1 (exclusive)
   */
  public randomExclusive(): number {
   ...
   */
  public uniqueInts(count: number, min: number, max: number): number[] {
    if (count > (max - min + 1)) {
...
// Create a singleton instance with a random seed
const defaultRNG = new MersenneTwister();

/**
 * Get a singleton instance of the Mersenne Twister
 * @returns The default Mersenne Twister instance
 */
export const getDefaultRNG = (): MersenneTwister => defaultRNG;

/**
 * Create a new Mersenne Twister instance with the specified seed
 * @param seed The seed value
 * @returns A new Mersenne Twister instance
 */
export const createRNG = (seed?: number): MersenneTwister => new MersenneTwister(seed);
