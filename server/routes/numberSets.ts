...
import { validateNumberSet, saveNumberSet, getUserNumberSets } from "../services/numberService.ts";
import { validateData } from "../utils/validation.ts";
import { validateNumberSetRequestSchema, saveNumberSetRequestSchema } from "../db/schema.ts";
  ...
  try {
    ...
    if (validatedData.numbers.length !== validatedData.quantity) {
      throw new BadRequestError(
        "Number of elements in 'numbers' array must match 'quantity'",
        { 
          expected: validatedData.quantity, 
          actual: validatedData.numbers.length 
        }
      );
    }
    ...
    if (uniqueNumbers.size !== validatedData.numbers.length) {
      throw new BadRequestError(
        "Numbers array contains duplicate values",
    ...
    if (outOfRangeNumbers.length > 0) {
      throw new BadRequestError(
        `Numbers must be between 1 and ${validatedData.maxValue}`,
        { outOfRangeNumbers }
      );
    }
 ...
 */
router.get("/users/:userId/number-sets", async (ctx: Context) => {
  try {
