...
export async function requestLogger(ctx: Context, next: Next) {
  ...
  // Log request details
  console.log(
    `[${requestId}] ${ctx.request.method} ${ctx.request.url.pathname} - Started`
  )
  ...
  try {
    ...
    console.log(
      `[${requestId}] ${ctx.request.method} ${ctx.request.url.pathname} - ${ctx.response.status} - ${ms}ms`
    )
  } catch (error) {
    // Log error but let error handler middleware handle it
    const ms = Date.now() - start
    console.error(
      `[${requestId}] ${ctx.request.method} ${ctx.request.url.pathname} - Error - ${ms}ms`
    )
    throw error
  }
}
