...
import { ConflictError, InternalServerError } from "../utils/errors.ts"
...
export async function validateNumberSet(
  groupId: string,
  numbers: number[],
  _quantity: number, // Not used directly but kept for API consistency
  _maxValue: number // Not used directly but kept for API consistency
): Promise<{
  isUnique: boolean
...
export async function saveNumberSet(
  ...
  if (!validation.isUnique) {
    throw new ConflictError(
      `This number set already exists in the group. ID: ${
        validation.existingSet?.id || "unknown"
      }`
    )
  }
  ...
  try {
    ...
    // Rethrow as InternalServerError with the original error as details
    if (error instanceof Error) {
      throw new InternalServerError("Failed to save number set", {
 ...
 */
export async function getGroupNumberSets(
  groupId: string
): Promise<Array<NumberSet & { user_display_name: string | null }>> {
  // Check if group exists
  await getGroupById(groupId)

  const numberSets = await query<
    NumberSet & { user_display_name: string | null }
  >(
 ...
 */
export async function getUserNumberSets(
  userId: string
): Promise<Array<NumberSet & { group_name: string }>> {
