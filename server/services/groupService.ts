...
export async function createInvitation(
  ...
  const invitationCode = generateRandomCode(config.security.invitationCodeLength);
 ...
 */
export async function getInvitationByCode(code: string): Promise<GroupInvitation> {
...
export async function joinGroup(
  ...
  return await transaction(async (client) => {
    ...
    // Get or create user
    let userResult = await client.queryObject<User>(
 ...
 */
export async function getGroupMembers(groupId: string): Promise<Array<GroupMember & { client_id: string; display_name: string | null }>> {
  // Check if group exists
  await getGroupById(groupId);
  
  const members = await query<GroupMember & { client_id: string; display_name: string | null }>(
