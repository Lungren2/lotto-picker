...
async function runCommand(cmd: string[], ignoreErrors = false): Promise<string> {
  try {
    ...
    if (!status.success && !ignoreErrors) {
...
async function checkDopplerConfiguration(): Promise<void> {
  try {
    ...
    console.error("❌ Failed to get Doppler configuration:", error.message);
  }
}
...
async function checkDopplerConnectivity(): Promise<void> {
  try {
    ...
    console.error("❌ Failed to connect to Doppler API:", error.message);
...
async function checkSecretsAccess(): Promise<void> {
  try {
    ...
    console.error("❌ Failed to access Doppler secrets:", error.message);
...
async function checkDopplerSetupFile(): Promise<void> {
  try {
    ...
    if (fileExists.includes("Error")) {
      ...
      console.error("   Create a doppler.yaml file with your project and config");
    } else {
    ...
    console.error("❌ Failed to check doppler.yaml file:", error.message);
  }
}
