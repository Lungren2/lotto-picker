...
const connectionString =
  Deno.env.get("NEON_CONNECTION_STRING") || config.db.connectionString

if (!connectionString) {
  console.error("Error: No database connection string provided.")
  console.error(
    "Please set the NEON_CONNECTION_STRING environment variable or update your config."
  )
  Deno.exit(1)
}
...
async function showDatabaseInfo() {
  ...
  try {
    ...
    console.log(
      `Schema 'lotto_picker' exists: ${
        schemaResult.rows.length > 0 ? "Yes" : "No"
      }`
    )
    ...
    const migrationsResult = await client
      .queryObject<{ name: string; applied_at: Date }>(
...
async function listTables() {
  ...
  try {
    const result = await client
      .queryObject<{ table_name: string; row_count: number }>(
        `SELECT
        t.table_name,
        (SELECT COUNT(*) FROM lotto_picker."${
          Deno.build.os === "windows" ? "$1" : "$1"
        }") AS row_count
  ...
  try {
    ...
    for (const row of tablesResult.rows) {
      ...
      await client.queryObject(
        `TRUNCATE TABLE lotto_picker."${row.table_name}" CASCADE`
      )
    }
...
async function showDatabaseStats() {
  ...
  try {
    ...
    const tableStatsResult = await client
      .queryObject<{ table_name: string; row_count: string }>(
        `SELECT
        table_name,
        (SELECT COUNT(*) FROM lotto_picker."${
          Deno.build.os === "windows" ? "$1" : "$1"
    ...
    const indexStatsResult = await client.queryObject<{
      table_name: string
      index_name: string
    }>(
      `SELECT
